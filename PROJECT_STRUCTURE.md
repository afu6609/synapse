...
- **支持配置项**: provider.baseUrl, provider.model, provider.apiKey, rerank.*, sliding-window.*
...
- **支持模式**:
  - `embedIndividually()`: 逐条向量化
  - `embedWithSlidingWindow()`: 滑动窗口向量化（含上下文）
...
### `application.yml`
- **默认配置**: 仅保留服务器端口、虚拟线程、滑动窗口、存储路径
- **运行时配置**: embedding/rerank 提供商需通过 `/api/v1/config` 设置
...
3. **正确的搜索流程**: rerank 在附近消息之前执行，避免上下文丢失
4. **异步事件广播**: 避免 WebSocket 并发发送冲突
5. **GraalVM 原生支持**: 启动时间 <1s，内存占用 <50MB
